...
fn xor_round_key(state: &mut [u8], keys: &[u8], round: usize) {
    xorr(&mut state[..16], &keys[round * 16..]);
}
...
fn shift_rows_inv(state: &mut [u8]) {
    let temp = state[..16].to_vec();
    for i in 0..16 {
        state[i] = temp[SHIFT_ROWS_TABLE_INV[i] as usize];
    }
}
...
fn mix_cols_inv(state: &mut [u8]) {
    mix_col_inv(&mut state[0..]);
    mix_col_inv(&mut state[4..]);
    mix_col_inv(&mut state[8..]);
    mix_col_inv(&mut state[12..]);
}

// This is normal oqs_aes128_enc_c, however they are using inv tables instead
pub fn oqs_mhy128_enc_c(plaintext: &[u8], schedule: &[u8]) -> Vec<u8> {
    // First Round
    let mut ciphertext = plaintext[..16].to_vec();
    xor_round_key(&mut ciphertext, schedule, 0);

    // Middle rounds
    for i in 0..9 {
        sub_bytes_inv(&mut ciphertext);
        shift_rows_inv(&mut ciphertext);
        mix_cols_inv(&mut ciphertext);
        xor_round_key(&mut ciphertext, schedule, i + 1);
    }

    // Final Round
    sub_bytes_inv(&mut ciphertext);
    shift_rows_inv(&mut ciphertext);
    xor_round_key(&mut ciphertext, schedule, 10);
